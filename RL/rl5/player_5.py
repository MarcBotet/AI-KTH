...
def epsilon_greedy(Q,
    ...
    if eps_type == 'constant':
        ...
        maxima = np.nanargmax(Q[state])
        aleatoria = np.random.choice(all_actions)
        acciones = [maxima, aleatoria]
        action = np.random.choice(acciones, 1, p=[(1 - epsilon), epsilon])[0]
        # ADD YOUR CODE SNIPPET BETWEEN EX 3.1
        ...
        value = scheduleLinear.value(current_total_steps)
        rd = np.random.random()
        if rd < value:
...
class PlayerControllerRL(PlayerController, FishesModelling):
    ...
    def q_learning(self):
        ...
        N = 5
        same_reward = False
        while episode <= self.episode_max and diff >= self.threshold and not same_reward:
            ...
            while not end_episode:
                ...
                action = epsilon_greedy(Q, s_current, list_pos, steps, self.epsilon_initial, self.epsilon_final, anneal_timesteps=self.annealing_timesteps)
            ...
            diff = np.absolute(np.nanmean(Q) - np.nanmean(Q_old))
            rewards.append(R_total)
            if len(rewards) > N:
                sub_rewards = rewards[-N:]
                max_reward = max(rewards)
                print("max_reward: ", max_reward)
                same_reward = all(max_reward == s for s in sub_rewards)
            # ADD YOUR CODE SNIPPET BETWEEN EX. 2.3
            Q_old[:] = Q
            print(
...
class ScheduleLinear(object):
    ...
    def value(self, t):
        ...
        valor = self.final_p - self.initial_p
        return self.initial_p + valor * (t / self.schedule_timesteps)
        # ADD YOUR CODE SNIPPET BETWEEN EX 3.2
